...
def img_scale_down_same_proportion(
        image: Image.Image, target_width: int, target_height: int) -> Image.Image:
    
    if not isinstance(image, Image.Image) or not isinstance(target_height, int) \
...
def convert_and_resize_images(
    dirpath: str, target_dirpath: str = "result", target_width: int = 1280, target_height: int = 960
) -> None:
    
    if not isinstance(dirpath, str) or not os.path.isdir(dirpath) \
        or not isinstance(target_height, int) or not isinstance(target_width, int):
        raise ValueError("\033[31mParameters are invalid\033[0m")
    ...
    for fname in os.listdir(dirpath):
        if fname.lower().endswith(".png"):
            ...
            img_resized = img_scale_down_same_proportion(img, target_width, target_height)
