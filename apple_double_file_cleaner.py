...
class AppleDoubleFilesCleanerStatus:
...
def apple_double_files_cleaner(dirpath: str,
    ...
    print(
        AppleDoubleFilesCleanerStatus.get_files_counter_message(_all_file_num)
    )

    print(
        AppleDoubleFilesCleanerStatus.get_op_result_message(_op_s_num, _op_f_num)
    )

    if move_res is not None:
        AppleDoubleFilesCleanerStatus.get_moved_failed_files(move_res)
